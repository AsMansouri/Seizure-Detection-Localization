function [ VectorConn, VectorConnstr ] = CN2( FilteredFFT, Powers, EpochTime, Thereshold_Corr, Regions, RegionLabels, Centers, Labels, Type, Electrodes, Seizures, Name, Visibility, Figures, FlagSource)
%function [ VectorConn, VectorConnstr ] = CN2( FilteredFFT, Powers, EpochTime, Thereshold_Corr, Regions, RegionLabels, Centers, Labels, Type, Electrodes, Seizures, Name, Visibility, Figures, FlagSource)
%   Plot the Correlation Network & Find the Average Connection and the
%   Strength of the Connections of the network
%   CoefCorr(j,k) = sqrt(((R(1,2)+1)/2).^2+mean([P(j) P(k)]).^2);
    ...
    for j = 1: length(Electrodes)
        for k =j+1:length(Electrodes)
            R = corrcoef(S(j,:),S(k,:));
            CoefCorr(j,k) = sqrt(((R(1,2)+1)/2).^2+mean([P(j) P(k)]).^2);
            CoefCorr(k,j) = sqrt(((R(1,2)+1)/2).^2+mean([P(j) P(k)]).^2);
        end
    end
